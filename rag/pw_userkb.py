...
PERSIST_DIR = os.getenv("PW_USERKB_PERSIST_DIR", "./state")
os.environ.setdefault("PATHWAY_PERSISTENT_STORAGE", PERSIST_DIR)
CACHE_DIR = os.getenv("PW_USERKB_CACHE_DIR", PERSIST_DIR)

LOG_DIR = os.getenv("PW_USERKB_LOG_DIR", "./logs")
LOG_FILE = os.path.join(LOG_DIR, "pw_userkb.log")
LOG_LEVEL = os.getenv("PW_USERKB_LOG_LEVEL", "INFO").upper()

HOST = os.getenv("PW_USERKB_HOST", "0.0.0.0")
PORT = _env_int("PW_USERKB_PORT", 4006)

DATA_DIR = os.getenv("PW_USERKB_DATA_DIR", "./user_uploads")
UPLOAD_MODE = _normalize_mode(os.getenv("PW_USERKB_UPLOAD_MODE", "streaming"))
OBJECT_PATTERN = os.getenv("PW_USERKB_OBJECT_PATTERN", "*")
AUTOCOMMIT_MS = _env_int("PW_USERKB_AUTOCOMMIT_MS", 500)

CHUNK_SIZE = _env_int("PW_USERKB_CHUNK_SIZE", 4000)
CHUNK_OVERLAP = _env_int("PW_USERKB_CHUNK_OVERLAP", 800)
DISABLE_SPLIT = _env_bool("PW_USERKB_DISABLE_SPLIT", False)

PARSER_MODE = os.getenv("PW_USERKB_PARSER_MODE", "paged")
PARSER_STRATEGY = os.getenv("PW_USERKB_PARSER_STRATEGY", "").strip()

EMBED_MODEL = os.getenv("PW_USERKB_EMBED_MODEL", "voyage/voyage-3-large")
EMBED_CAPACITY = _env_int("PW_USERKB_EMBED_CAPACITY", 10)
EMBED_CACHE_SIZE = _env_int("PW_USERKB_EMBED_CACHE_SIZE", 2**30)
PARSER_CACHE_SIZE = _env_int("PW_USERKB_PARSER_CACHE_SIZE", 2**30)

INDEX_MODE = os.getenv("PW_USERKB_INDEX_MODE", "hybrid").strip().lower()
RRF_K = _env_float("PW_USERKB_RRF_K", 60.0)
BM25_RAM_BUDGET = _env_int("PW_USERKB_BM25_RAM_BUDGET", 524_288_000)
BM25_IN_MEMORY = _env_bool("PW_USERKB_BM25_IN_MEMORY", True)
ENABLE_CACHE = _env_bool("PW_USERKB_ENABLE_CACHE", True)

USEARCH_RESERVED = _env_int("PW_USERKB_USEARCH_RESERVED", 400)
USEARCH_CONNECTIVITY = _env_int("PW_USERKB_USEARCH_CONNECTIVITY", 0)
USEARCH_EXPANSION_ADD = _env_int("PW_USERKB_USEARCH_EXPANSION_ADD", 0)
USEARCH_EXPANSION_SEARCH = _env_int("PW_USERKB_USEARCH_EXPANSION_SEARCH", 0)

MAX_DOC_CHARS = _env_int("PW_USERKB_MAX_DOC_CHARS", 0)
...
def _log_uncaught_exceptions(exc_type, exc, tb):
    if issubclass(exc_type, KeyboardInterrupt):
        ...
        return
    logging.getLogger("pw_userkb").exception("Uncaught exception", exc_info=(exc_type, exc, tb))


def _log_thread_exceptions(args):
    logging.getLogger("pw_userkb").exception(
...
logger = logging.getLogger("pw_userkb")
...
class DocumentProcessor:
    """
    Processes user-uploaded documents and serves them via a Pathway DocumentStoreServer.
    Watches ./user_uploads for new files and indexes them with the configured embedder/index.
    """
    ...
    def __init__(self, host: str = HOST, port: int = PORT):
        ...
        if self.index_mode != INDEX_MODE:
            logger.warning("Unknown PW_USERKB_INDEX_MODE=%s; defaulting to hybrid", INDEX_MODE)

        parser_mode = PARSER_MODE if PARSER_MODE in {"single", "elements", "paged"} else "paged"
        if parser_mode != PARSER_MODE:
            logger.warning("Unknown PW_USERKB_PARSER_MODE=%s; defaulting to paged", PARSER_MODE)
    ...
    def initialize_vector_store(self, path1):
        """Initialize document store watching the given directory."""
        ...
        source1 = pw.io.fs.read(
            ...
            persistent_id="pw_userkb_uploads",
        )
    ...
    def setup_document_server(self):
        """Configure the DocumentStoreServer using self.host and self.port."""
...
def main():
    ...
    try:
        logger.info("Starting Pathway pipeline (user KB)")
        processor.run()
        logger.info("Pathway pipeline stopped")
    except KeyboardInterrupt:
        logger.info("Shutting down server...")
    except Exception:
        logger.exception("pw_userkb crashed")
