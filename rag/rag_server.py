...
load_dotenv('.env')
app = FastAPI()
RAG_DEFAULT_MODEL = os.getenv("RAG_DEFAULT_MODEL", "deepseek-chat")
...
client = get_client(RAG_DEFAULT_MODEL)
...
def generate_answer_openai(query: str, source: str, retrieved_docs: List[Dict[str, Any]], max_tokens: int = 1000, model: str = None) -> str:
    """
    Generate an answer using OpenAI model
    """
    model = model or RAG_DEFAULT_MODEL
    try:
...
def generate(query_request: Query):
    ...
    answer = generate_answer_openai(
        ...
        query_request.model or RAG_DEFAULT_MODEL,
    )
